...
rm ../result/banana/set_threads/*.txt
for loop in true false
do
#  value=$(awk -v j_awk="$j" 'BEGIN {value_awk=j_awk/10.0; printf "%.1f", value_awk}')
  sed -i "s:^set_threads=.*[a-zA-Z]$:set_threads=$loop:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
  sed -i "s:^pure_GA_path.*\.txt$:pure_GA_path=\.\./result/banana/set_threads/$loop.txt:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
  for((i=0;i<$[run_num];i++)) 
  do  
