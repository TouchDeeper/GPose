...
sed -i "s:^mutation_method=.*[a-zA-Z]$:mutation_method=uniform:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini

cd /home/wang/imta_project/objectPoseEstimation/src/src/localPipeline/cmake-build-debug/
rm ../result/banana/fitness/*.txt
for loop in hard_code_inlier_fraction no_hard_code
do
  sed -i "s:^fitness_method=.*[a-zA-Z]$:fitness_method=$loop:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
  sed -i "s:^output_path.*\.txt$:output_path=\.\./result/banana/fitness/$loop.txt:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
  for((i=0;i<$[run_num];i++)) 
  do  
