...
rm ../result/banana/population_local/*.txt
for((j=0;j<16;j++))
do
  let value=50+j*10
  sed -i "s:^population=.*[0-9]$:population=$value:g" ~/imta_project/pose_estimation/config.ini
  sed -i "s:^pure_GA_path.*\.txt$:pure_GA_path=\.\./result/banana/population_local/$value.txt:g" ~/imta_project/pose_estimation/config.ini
  for((i=0;i<$[run_num];i++)) 
  do  
