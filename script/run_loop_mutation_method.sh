...
#  value=$(awk -v j_awk="$j" 'BEGIN {value_awk=j_awk/10.0; printf "%.1f", value_awk}')
  ...
  sed -i "s:^pure_GA_path.*\.txt$:pure_GA_path=\.\./result/banana/mutation/$loop.txt:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
  for((i=0;i<$[run_num];i++)) 
  do  
