...
#    sed -i "s:^F0=.*[0-9]$:F0=0.4:g" ${config_path}
    exit
}
...
rm ../result/banana/real/local/mutation/*/*.txt
sed -i "s:^model_name=.*[a-zA-Z]$:model_name=banana:g" ${config_path}
sed -i "s:^scene_name=.*[0-9]$:scene_name=scene1:g" ${config_path}

for loop in non_uniform uniform mutate_by_chromosomes
do
#  value=$(awk -v j_awk="$j" 'BEGIN {value_awk=j_awk/10.0; printf "%.1f", value_awk}')
  sed -i "s:^mutation_method=.*[a-zA-Z]$:mutation_method=$loop:g" ${config_path}
  sed -i "s:^pure_GA_path.*\.txt$:pure_GA_path=real/local/mutation/$loop/pure_GA.txt:g" ${config_path}
  sed -i "s:^pure_icp_path.*\.txt$:pure_icp_path=real/local/mutation/$loop/pure_icp.txt:g" ${config_path}
  sed -i "s:^GA_icp_path.*\.txt$:GA_icp_path=real/local/mutation/$loop/GA_icp.txt:g" ${config_path}
