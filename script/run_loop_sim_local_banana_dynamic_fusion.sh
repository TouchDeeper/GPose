...
rm ../result/banana/sim/local/dynamic_fusion/*/*.txt
sed -i "s:^model_name=.*[a-zA-Z]$:model_name=banana:g" ${config_path}
sed -i "s:^scene_name=.*[0-9]$:scene_name=sim1:g" ${config_path}

for loop in  dynamic_fusion static_fusion complete_model no_fusion
do
  sed -i "s:^fusion_mode=.*[a-zA-Z]$:fusion_mode=$loop:g" ${config_path}
  sed -i "s:^pure_GA_path.*\.txt$:pure_GA_path=sim/local/dynamic_fusion/$loop/pure_GA.txt:g" ${config_path}
  sed -i "s:^pure_icp_path.*\.txt$:pure_icp_path=sim/local/dynamic_fusion/$loop/pure_icp.txt:g" ${config_path}
  sed -i "s:^GA_icp_path.*\.txt$:GA_icp_path=sim/local/dynamic_fusion/$loop/GA_icp.txt:g" ${config_path}
