...
rm ../result/banana/sim/local/mutation/*/*.txt
...
#  value=$(awk -v j_awk="$j" 'BEGIN {value_awk=j_awk/10.0; printf "%.1f", value_awk}')
  ...
  sed -i "s:^pure_GA_path.*\.txt$:pure_GA_path=sim/local/mutation/$loop/pure_GA.txt:g" ${config_path}
  sed -i "s:^pure_icp_path.*\.txt$:pure_icp_path=sim/local/mutation/$loop/pure_icp.txt:g" ${config_path}
  sed -i "s:^GA_icp_path.*\.txt$:GA_icp_path=sim/local/mutation/$loop/GA_icp.txt:g" ${config_path}
