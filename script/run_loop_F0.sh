...
function onCtrlC () {
    ...
    sed -i "s:^F0=.*[0-9]$:F0=0.3:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
    exit
}
...
sed -i "s:^visual=.*[a-zA-Z]$:visual=false:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
sed -i "s:^mutation_method=.*[a-zA-Z]$:mutation_method=mutate_by_chromosomes:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini

cd /home/wang/imta_project/objectPoseEstimation/src/src/localPipeline/cmake-build-debug/
rm ../result/banana/F0/*.txt
for((j=1;j<6;j++))
do
  value=$(awk -v j_awk="$j" 'BEGIN {value_awk=j_awk/10.0; printf "%.1f", value_awk}')
  sed -i "s:^F0=.*[0-9]$:F0=$value:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
  sed -i "s:^pure_GA_path.*\.txt$:pure_GA_path=\.\./result/banana/F0/$j.txt:g" ~/imta_project/objectPoseEstimation/src/src/localPipeline/config.ini
  for((i=0;i<$[run_num];i++)) 
  do  
   ...
   ../script/is_globalPipeline_exist.sh
  done 
