...
rm ../result/banana/whole_pipe/*/*.txt
#rm ../result/joystick/whole_pipe/*/*.txt


for model in banana #joystick
do
    sed -i "s:^model_name=.*[a-zA-Z]$:model_name=$model:g" ${config_path}
    for scene in sim1 scene1
    do
        sed -i "s:^scene_name=.*[0-9]$:scene_name=$scene:g" ${config_path}
        sed -i "s:^pure_GA_path.*\.txt$:pure_GA_path=whole_pipe/$scene/pure_GA.txt:g" ${config_path}
        sed -i "s:^pure_icp_path.*\.txt$:pure_icp_path=whole_pipe/$scene/pure_icp.txt:g" ${config_path}
        sed -i "s:^GA_icp_path.*\.txt$:GA_icp_path=whole_pipe/$scene/GA_icp.txt:g" ${config_path}
        sed -i "s:^time_path.*\.txt$:time_path=whole_pipe/$scene/time.txt:g" ${config_path}
